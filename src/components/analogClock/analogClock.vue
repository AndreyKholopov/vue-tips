<template>
  <div ref="clock" class="clock">
    <svg ref="clockFace" class="clock-face">
      <use xlink:href="./analogClock.svg#clockFace"></use>
    </svg>
    <div class="hour">
...
export default {
  ...
  mounted() {
    ...
    // set all colors
    this.$refs.clock.style.setProperty(
      "--main-clock-color",
      `${this.color ? this.color : "rgb(255, 255, 255)"}`
    );
    this.$refs.clockFace.style.setProperty(
      "--clock-face-color",
      `${this.faceColor ? this.faceColor : "#000"}`
    );

    // set width and height
    this.$refs.clock.style.setProperty(
      "--clock-width-color",
      `${this.width ? `${this.width}px` : "350px"}`
    );
  },
};
</script>
